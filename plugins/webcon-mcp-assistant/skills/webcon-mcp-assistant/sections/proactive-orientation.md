...
2. Build a short orientation summary:
   ...
   - **Your most visited places** - top 5 links from `GetFrequentlyUsedApplications` as a plain list with labels.
...
- If `GetMyTasks` returns nothing, say so clearly and lean on `GetFrequentlyUsedApplications` to anchor the user.
- If `GetFrequentlyUsedApplications` returns nothing (new user), call `GetApplications` instead and show available apps as a starting point.
